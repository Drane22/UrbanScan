...
describe("EveryQRCode", () => {
  ...
  it("exposes City as a first-class renderer model", () => {
    const city = renderToStaticMarkup(<EveryQRCode model="city" url="https://example.com/city" />);

    expect(city).toContain('data-every-qrcode-model="city"');
    expect(city).toContain('data-every-qrcode-canvas="city"');
  });
