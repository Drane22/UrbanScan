...
it("loads only the shader bundle for the selected renderer model", async () => {
  ...
  expect(city).toMatchObject({ form: "city" });
...
it("uses an instanced multi-part City pipeline with a staged reversible QR morph", () => {
  expect(CITY_SHADER).toContain("@builtin(instance_index) instanceIndex");
  expect(CITY_SHADER).toContain("const CITY_PARTS: u32 = 4u");
  expect(CITY_SHADER).toContain("let propStage = 1.0 - cityStage(0.0, 0.18)");
  expect(CITY_SHADER).toContain("let heightStage = 1.0 - cityStage(0.22, 0.62)");
  expect(CITY_SHADER).toContain("let footStage = cityStage(0.55, 0.9)");
  expect(CITY_SHADER).toContain("let camera = cityStage(0.5, 1.0)");
  expect(CITY_SHADER).toContain("@binding(4) var<storage, read> cityLots");
  expect(CITY_SHADER).toContain("uniforms.camera.x");
});

it("keeps City scene data lightweight instead of generating unused Tree geometry", async () => {
  const model = await seedModel.createSeedModel(
    await createEveryQRCodeIdentity("https://example.com/lightweight-city"),
  );
  const scene = createSeedGpuScene(model, "city");

  expect(scene.segmentCount).toBe(0);
  expect(scene.flowerCount).toBe(0);
  expect(scene.grassCount).toBe(0);
  expect(scene.rainCount).toBeGreaterThan(0);
});
